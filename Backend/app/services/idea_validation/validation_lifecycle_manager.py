...
from app.services.idea_validation.validation_coordinator import ValidationCoordinator
from app.services.idea_validation.idea_manager import IdeaManager
...
class ValidationLifecycleManager:
    ...
    def __init__(self):
        self.orchestrator = ValidationCoordinator()
        self.validations_collection = validation_results_collection
    
    async def run_validation_sync(
        self,
        idea_id: str,
        user_id: str,
        ...
        """
        Run validation SYNCHRONOUSLY (waits for completion)
        
        This method runs the complete validation and returns the final result.
        Use this when you want to wait for validation to complete before returning.
        ...
        Returns:
            Complete ValidationResultResponse with all scores
        """
        # Generate validation ID
        validation_id = str(uuid.uuid4())
        
        # Verify idea exists and belongs to user
        idea = IdeaManager.get_idea(user_id, idea_id)
        if not idea:
            raise ValueError(f"Idea {idea_id} not found or does not belong to user")
        
        # Create validation record with IN_PROGRESS status
        validation_record = {
        ...
        logger.info(f"Starting validation {validation_id} for idea {idea_id}")
        ...
        try:
            ...
            logger.info(f"Validation completed for idea {idea_id}")
    ...
    async def compare_validations(
        ...
        for validation in validations:
            idea = IdeaManager.get_idea(user_id, validation.idea_id)
            if idea:
                ideas_data.append({
                    "idea_id": idea.id,
    ...
    async def export_validation_json(
        ...
        - Idea information
        - Metadata (validation date, version)
    ...
    async def get_shared_validation(
        ...
        # Get idea details
        from app.services.idea_validation.idea_manager import IdeaManager
        idea = IdeaManager.get_idea(user_id, validation_doc["idea_id"])
