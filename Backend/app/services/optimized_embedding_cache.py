...
class OptimizedEmbeddingCache:
    ...
    def _find_semantic_match(self, embedding: np.ndarray) -> Optional[str]:
        """
        Optimized semantic similarity search with efficient numpy-based computation
        Returns hash of similar embedding if found above configurable threshold
        
        Performance optimizations:
        - Pre-normalized embeddings stored in index for faster computation
        - Vectorized numpy operations with float32 precision
        - Direct dot product for cosine similarity (since embeddings are normalized)
        - Early termination and efficient memory usage
        - Performance tracking for monitoring
        """
        if not self.similarity_index["embeddings"]:
            return None
        ...
        try:
            # Ensure input embedding is normalized for efficient cosine similarity
            embedding_norm = np.linalg.norm(embedding)
            if embedding_norm == 0:
                logger.warning("Zero-norm embedding provided for semantic search")
                return None
            
            normalized_embedding = embedding / embedding_norm
            
            # Convert stored embeddings to optimized numpy matrix
            # Embeddings are already normalized when stored, so no need to normalize again
            cached_embeddings = np.array(self.similarity_index["embeddings"], dtype=np.float32)
            
            if cached_embeddings.size == 0:
                return None
            
            # Efficient cosine similarity using direct dot product
            # Since both input and cached embeddings are normalized, dot product = cosine similarity
            similarities = np.dot(cached_embeddings, normalized_embedding.astype(np.float32))
            
            # Find best match above configurable threshold
            max_idx = np.argmax(similarities)
            max_similarity = similarities[max_idx]
            
            if max_similarity >= self.similarity_threshold:
                matched_hash = self.similarity_index["hashes"][max_idx]
                logger.info(f"Semantic cache hit: {max_similarity:.3f} similarity (threshold: {self.similarity_threshold})")
                return matched_hash
            
            # Log near-misses for debugging (only if close to threshold)
            if max_similarity >= self.similarity_threshold - 0.05:
                logger.debug(f"Near semantic match: {max_similarity:.3f} similarity (below threshold: {self.similarity_threshold})")
            
        except Exception as e:
            logger.warning(f"Error in optimized semantic search: {e}")
        
        finally:
            # Update semantic search performance metrics
    ...
    def _add_to_semantic_index(self, text: str, embedding: np.ndarray, hash_key: str):
        """
        Add embedding to semantic similarity index with FIFO cache management
        
        Implements efficient FIFO eviction to prevent memory issues while
        maintaining optimal cache performance for recent embeddings.
        """
        try:
            ...
            if hash_key in self.similarity_index["hashes"]:
                logger.debug(f"Embedding already exists in similarity index: {hash_key}")
                return
            
            # FIFO cache management - remove oldest entries when at capacity
            if len(self.similarity_index["embeddings"]) >= self.max_cache_size:
                ...
                entries_to_remove = min(batch_size, len(self.similarity_index["embeddings"]))
                
                logger.debug(f"FIFO eviction: removing {entries_to_remove} oldest entries from similarity index")
                ...
                for _ in range(entries_to_remove):
                    self.similarity_index["embeddings"].pop(0)
                    self.similarity_index["texts"].pop(0) 
                    self.similarity_index["hashes"].pop(0)
            ...
            # Add new entry with normalized embedding for efficiency
            self.similarity_index["embeddings"].append(normalized_embedding.astype(np.float32).tolist())
            self.similarity_index["texts"].append(text[:200])  # Truncate for storage efficiency
            self.similarity_index["hashes"].append(hash_key)
