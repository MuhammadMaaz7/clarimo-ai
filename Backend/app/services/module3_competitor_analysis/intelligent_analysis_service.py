...
import numpy as np
from .local_analysis_service import LocalAnalysisService
...
class IntelligentAnalysisService:
    ...
    async def analyze_competitors(
        ...
        if use_local_analysis and len(competitors) > 0:
            ...
            local_insights = await LocalAnalysisService.analyze_all_competitors(
        ...
        stats = IntelligentAnalysisService._get_statistics(competitors)
        
        # Step 3: Rank & Filter Top Competitors for detailed LLM analysis
        top_competitors = IntelligentAnalysisService._rank_and_filter(
            competitors,
            max_competitors_for_llm
        )
        
        # Step 4: Aggregate Features & Pricing
        all_features = IntelligentAnalysisService._aggregate_features(competitors)
        pricing_info = IntelligentAnalysisService._aggregate_pricing(competitors)
