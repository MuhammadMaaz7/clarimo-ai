...
class LocalAnalysisService:
    ...
    async def analyze_all_competitors(
        ...
        competitor_texts = LocalAnalysisService._extract_texts(competitors)
        
        # 2. TF-IDF Analysis - Find important keywords
        tfidf_insights = LocalAnalysisService._tfidf_analysis(competitor_texts)
        
        # 3. Topic Modeling - Discover themes
        topics = LocalAnalysisService._topic_modeling(competitor_texts)
        
        # 4. Similarity Clustering
        clusters = LocalAnalysisService._similarity_clustering(
            product_info, 
            competitors, 
            competitor_texts
        )
        ...
        feature_insights = LocalAnalysisService._extract_features(competitors)
        
        # 6. Pricing Analysis
        pricing_insights = LocalAnalysisService._analyze_pricing(competitors)
        
        # 7. Source Analysis
        source_insights = LocalAnalysisService._analyze_sources(competitors)
        
        # 8. Generate Condensed Summary
        summary = LocalAnalysisService._generate_summary(
        ...
        try:
            ...
            for topic_idx, topic in enumerate(lda.components_):
                ...
                topics.append({
                    ...
                    "keywords": top_words,
                    "theme": LocalAnalysisService._infer_theme(top_words)
                })
