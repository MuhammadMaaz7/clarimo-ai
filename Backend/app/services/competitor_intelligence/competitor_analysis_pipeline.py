...
from .google_competitor_finder import get_google_finder
from .producthunt_data_fetcher import get_producthunt_fetcher
from app.services.idea_validation.market_data_fetcher import MarketDataFetcher
from .competitor_web_scraper import get_web_scraper
from .hybrid_analysis_engine import HybridAnalysisEngine
from .llm_insights_generator import LLMInsightsGenerator
...
class CompetitorAnalysisPipeline:
    ...
    async def analyze_product(
        ...
        try:
            ...
            keywords = await CompetitorAnalysisPipeline._generate_keywords(product_info)
            logger.info(f"✓ Generated {len(keywords)} keywords: {', '.join(keywords)}")
            if user_id:
                CompetitorAnalysisPipeline._save_step_data(user_id, analysis_id, "1_keywords", {"keywords": keywords})
            ...
            raw_competitors = await CompetitorAnalysisPipeline._fetch_competitors_from_apis(
                product_info, keywords
            )
            logger.info(f"✓ Fetched {len(raw_competitors)} raw competitors")
            if user_id:
                CompetitorAnalysisPipeline._save_step_data(user_id, analysis_id, "2_raw_competitors", {"competitors": raw_competitors, "count": len(raw_competitors)})
            ...
            filtered_competitors = CompetitorAnalysisPipeline._filter_non_competitors(raw_competitors)
            logger.info(f"✓ After filtering: {len(filtered_competitors)} valid competitors")
            if user_id:
                CompetitorAnalysisPipeline._save_step_data(user_id, analysis_id, "3_filtered_competitors", {"competitors": filtered_competitors, "count": len(filtered_competitors)})
            ...
            classified_competitors = CompetitorAnalysisPipeline._classify_competitors(
            ...
            if user_id:
                CompetitorAnalysisPipeline._save_step_data(user_id, analysis_id, "4_classified_competitors", {
                    "competitors": classified_competitors,
                    "direct_count": sum(1 for c in classified_competitors if c.get('competitor_type') == 'direct'),
                    "indirect_count": sum(1 for c in classified_competitors if c.get('competitor_type') == 'indirect')
                })
            
            # Step 5: LLM Verification - Select Top 5 Competitors
            logger.info("Step 5/7: LLM verification - selecting top 5 competitors...")
            top_5_competitors = await CompetitorAnalysisPipeline._llm_select_top_competitors(
                product_info, classified_competitors
            )
            logger.info(f"✓ LLM selected top {len(top_5_competitors)} competitors")
            if user_id:
                CompetitorAnalysisPipeline._save_step_data(user_id, analysis_id, "5_top_competitors", {"competitors": top_5_competitors, "count": len(top_5_competitors)})
            ...
            enriched_top_5 = await CompetitorAnalysisPipeline._scrape_top_competitors(top_5_competitors)
            logger.info(f"✓ Scraped {sum(1 for c in enriched_top_5 if c.get('enriched'))} competitors")
            if user_id:
                CompetitorAnalysisPipeline._save_step_data(user_id, analysis_id, "6_enriched_competitors", {"competitors": enriched_top_5, "enriched_count": sum(1 for c in enriched_top_5 if c.get('enriched'))})
            ...
            final_analysis = await CompetitorAnalysisPipeline._llm_structure_output(
                product_info, enriched_top_5, classified_competitors
            )
            logger.info(f"✓ Analysis structured for dashboard")
            if user_id:
                CompetitorAnalysisPipeline._save_step_data(user_id, analysis_id, "7_final_analysis", final_analysis)
            
            # Build Final Response
            response = await CompetitorAnalysisPipeline._build_clean_response(
            ...
            if save_to_db and user_id:
                try:
                    CompetitorAnalysisPipeline._save_to_database(
    ...
    async def _generate_keywords(product_info: Dict[str, Any]) -> List[str]:
        ...
        try:
            result = await KeywordGenerator.generate_competitor_keywords(
    ...
    async def _fetch_competitors_from_apis(
        ...
        tasks = [
            CompetitorAnalysisPipeline._fetch_product_hunt(keywords),
            CompetitorAnalysisPipeline._fetch_google(
                product_info['name'], 
                keywords, 
                product_info.get('description', '')
            ),
            CompetitorAnalysisPipeline._fetch_github(keywords),
            CompetitorAnalysisPipeline._fetch_app_stores(keywords)
        ]
    ...
    async def _fetch_product_hunt(keywords: List[str]) -> Dict[str, Any]:
        ...
        try:
            service = get_producthunt_fetcher()
            ...
            for p in result.get('products', []):
                competitors.append({
                    "name": p['name'],
                    "description": truncate_at_sentence(p.get('description', ''), 300),
                    "url": p['url'],
                    "source": "product_hunt",
                    "votes": p.get('votes'),
                    "topics": p.get('topics', [])
    ...
    async def _fetch_github(keywords: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            for repo in result.get('repositories', []):
                competitors.append({
                    "name": repo['name'],
                    "description": truncate_at_sentence(repo.get('description', ''), 300),
                    "url": repo['url'],
                    "source": "github",
        ...
        for comp in top_candidates:
            ...
            try:
                ...
                if result.get('scrape_success') and result.get('features'):
                    ...
                    llm_result = await CompetitorAnalysisPipeline._llm_extract_features_from_description(
                        comp['name'],
...
Return ONLY the JSON, nothing else:"""
    ...
    async def _llm_structure_output(
        ...
        try:
            ...
            llm_analysis = await LLMInsightsGenerator.generate_competitive_analysis(
                product_info=product_info,
    ...
    async def _build_clean_response(
        ...
        feature_matrix = await CompetitorAnalysisPipeline._build_feature_matrix(
            product_info, top_5_enriched
        )
        
        # Build comparison data
        comparison = CompetitorAnalysisPipeline._build_comparison(
            product_info, top_5_enriched, llm_analysis
        )
        
        # Build gap analysis
        gap_analysis = CompetitorAnalysisPipeline._build_gap_analysis(llm_analysis)
    ...
    async def _build_feature_matrix(
        ...
        """
        from .feature_comparison_builder import FeatureComparisonBuilder
        
        try:
            # Use the LLM-based feature matrix service
            matrix = await FeatureComparisonBuilder.build_feature_matrix(
    ...
    @staticmethod
    def _save_step_data(user_id: str, analysis_id: str, step_name: str, data: Dict[str, Any]):
        """Save intermediate step data to files"""
        import json
        from pathlib import Path
        
        try:
            base_dir = Path("data/competitor_analysis")
            user_dir = base_dir / user_id / analysis_id
            user_dir.mkdir(parents=True, exist_ok=True)
            
            step_file = user_dir / f"{step_name}.json"
            with open(step_file, 'w', encoding='utf-8') as f:
                # Convert to JSON-serializable format
                serializable_data = json.loads(json.dumps(data, default=str))
                json.dump(serializable_data, f, indent=2, ensure_ascii=False)
            
            logger.debug(f"✓ Saved step data: {step_name}")
        except Exception as e:
            logger.error(f"Failed to save step data {step_name}: {str(e)}")
    ...
    def _save_to_database(
        ...
        """Save analysis to database and files (synchronous)"""
        import json
        from pathlib import Path
