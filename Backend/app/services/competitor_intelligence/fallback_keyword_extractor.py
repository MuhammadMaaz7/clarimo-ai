...
class FallbackKeywordExtractor:
    ...
    def extract_keywords(
        ...
        # e.g., "TaskMaster" -> "task"
        name_phrases = FallbackKeywordExtractor._extract_phrases_from_name(product_name)
        keywords.extend(name_phrases)
        
        # 2. Extract key noun phrases from description (2-3 word combinations)
        desc_phrases = FallbackKeywordExtractor._extract_noun_phrases(product_description)
        keywords.extend(desc_phrases[:3])  # Top 3 phrases
        
        # 3. Extract category keywords from description
        desc_lower = product_description.lower()
        for category_keyword in FallbackKeywordExtractor.CATEGORY_KEYWORDS:
        ...
        for feature in key_features[:3]:  # Top 3 features
            ...
            if any(term in feature_lower for term in ['editor', 'manager', 'tool', 'tracker', 'organizer']):
                # Extract the phrase
                words = FallbackKeywordExtractor._extract_words(feature)
                if len(words) >= 2:
                    keywords.append(f"{words[0]} {words[1]}".lower())
        
        # 5. Fallback: Extract important single words if we don't have enough
        if len(keywords) < 3:
            desc_words = FallbackKeywordExtractor._extract_words(product_description)
            for word in desc_words[:30]:
                word_lower = word.lower()
                if (len(word) > 5 and 
                    word_lower not in FallbackKeywordExtractor.STOP_WORDS and
        ...
        # Prioritize keywords
        prioritized = FallbackKeywordExtractor._prioritize_keywords(
            unique_keywords,
    ...
    def _extract_phrases_from_name(product_name: str) -> List[str]:
        ...
        for word in words:
            if len(word) > 4 and word.lower() not in FallbackKeywordExtractor.STOP_WORDS:
    ...
    def _extract_noun_phrases(text: str) -> List[str]:
        ...
        """
        phrases = []
        words = FallbackKeywordExtractor._extract_words(text)
        ...
        for i in range(len(words) - 1):
            ...
            if (word1 in FallbackKeywordExtractor.STOP_WORDS or 
                word2 in FallbackKeywordExtractor.STOP_WORDS or
    ...
    def _prioritize_keywords(keywords: List[str], description: str) -> List[str]:
        ...
        for keyword in keywords:
            ...
            if keyword in FallbackKeywordExtractor.TECH_KEYWORDS:
                score += 100
            
            # Category keywords get high priority
            if keyword in FallbackKeywordExtractor.CATEGORY_KEYWORDS:
                score += 50
    ...
    def extract_category(product_description: str) -> str:
        ...
        for category in FallbackKeywordExtractor.CATEGORY_KEYWORDS:
