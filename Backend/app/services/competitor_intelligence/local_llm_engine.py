...
class LocalLLMEngine:
    ...
    def load_model(model_name: Optional[str] = None):
        ...
        """
        if LocalLLMEngine._pipeline is not None:
            return LocalLLMEngine._pipeline
        
        if not LocalLLMEngine.is_available():
        ...
        try:
            ...
            # Load pipeline
            LocalLLMEngine._pipeline = pipeline(
            ...
            LocalLLMEngine._model_name = model_name
    ...
    async def generate_competitive_analysis(
        ...
        """
        pipeline = LocalLLMEngine.load_model(model_name)
        ...
        try:
            ...
            # 1. Market Position
            market_prompt = LocalLLMEngine._build_market_position_prompt(
                product_info, preprocessed_data
            )
            analysis['market_position'] = LocalLLMEngine._generate(
                pipeline, market_prompt
            )
            
            # 2. Key Competitors
            analysis['key_competitors'] = LocalLLMEngine._extract_key_competitors(
                preprocessed_data
            )
            
            # 3. Competitive Advantages
            advantages_prompt = LocalLLMEngine._build_advantages_prompt(
                product_info, preprocessed_data
            )
            advantages_text = LocalLLMEngine._generate(pipeline, advantages_prompt)
            analysis['competitive_advantages'] = LocalLLMEngine._parse_list(advantages_text)
            
            # 4. Competitive Threats
            threats_prompt = LocalLLMEngine._build_threats_prompt(preprocessed_data)
            threats_text = LocalLLMEngine._generate(pipeline, threats_prompt)
            analysis['competitive_threats'] = LocalLLMEngine._parse_list(threats_text)
            
            # 5. Feature Comparison (from local insights)
            analysis['feature_comparison'] = LocalLLMEngine._build_feature_comparison(
                product_info, preprocessed_data
            )
            
            # 6. Gap Analysis
            analysis['gap_analysis'] = LocalLLMEngine._build_gap_analysis(
                preprocessed_data
            )
            
            # 7. Differentiation Strategy
            diff_prompt = LocalLLMEngine._build_differentiation_prompt(
                product_info, preprocessed_data
            )
            analysis['differentiation_strategy'] = LocalLLMEngine._generate(
                pipeline, diff_prompt
            )
            
            # 8. Pricing Strategy
            pricing_prompt = LocalLLMEngine._build_pricing_prompt(
                product_info, preprocessed_data
            )
            analysis['pricing_strategy'] = LocalLLMEngine._generate(
                pipeline, pricing_prompt
            )
            
            # 9. Target Audience
            audience_prompt = LocalLLMEngine._build_audience_prompt(
                product_info, preprocessed_data
            )
            analysis['target_audience_insights'] = LocalLLMEngine._generate(
                pipeline, audience_prompt
            )
            
            # 10. Opportunity Score
            analysis['opportunity_score'] = LocalLLMEngine._calculate_opportunity_score(
                preprocessed_data
            )
            
            analysis['analysis_method'] = 'huggingface_cpu'
            analysis['model_used'] = LocalLLMEngine._model_name
