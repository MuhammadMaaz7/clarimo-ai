...
@router.post("/validate", response_model=ValidationResultResponse, status_code=status.HTTP_200_OK)
async def validate_idea(
    validation_request: ValidationResultCreate,
    current_user: dict = Depends(get_current_user)
):
    """
    Validate an idea (SYNCHRONOUS - waits for completion)
    
    This endpoint runs the complete validation process and returns the final result.
    The request will take 30-60 seconds to complete as it performs LLM-based analysis.
    
    - **idea_id**: ID of the idea to validate
    - **config**: Optional validation configuration
    
    Returns:
        Complete ValidationResultResponse with all scores and analysis
    """
    try:
        ...
        validation_result = await validation_service.run_validation_sync(
        ...
        logger.error(f"Failed to validate idea: {str(e)}")
        raise HTTPException(
...
@router.post("/validate-async", response_model=ValidationResultResponse, status_code=status.HTTP_202_ACCEPTED)
async def validate_idea_async(
    validation_request: ValidationResultCreate,
    background_tasks: BackgroundTasks,
    current_user: dict = Depends(get_current_user)
):
    """
    Start validation for an idea (ASYNC - returns immediately)
    ...
    Use this if you want to implement polling or WebSocket updates.
